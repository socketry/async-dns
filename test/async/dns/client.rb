...
describe Async::DNS::Client do
	...
	it "should result in non-existent domain" do
		response = client.query('foobar.example.com', Resolv::DNS::Resource::IN::A)
		
		expect(response.rcode).to be == Resolv::DNS::RCode::NXDomain
	end
	
	it "should result in some answers" do
		response = client.query('google.com', Resolv::DNS::Resource::IN::A)
	...
	with '#addresses_for' do
		it "should return IP addresses" do
			addresses = client.addresses_for('google.com')
		...
		it "should recursively resolve CNAME records" do
			...
			addresses = client.addresses_for('www.baidu.com')
	...
	with '#fully_qualified_name' do
		let(:client) {Async::DNS::Client.new(origin: "foo.bar.")}
		
		it "should generate fully qualified domain name with specified origin" do
			fully_qualified_name = client.fully_qualified_name("baz")
